...
@section('content')
    <div class="max-w-2xl mx-auto py-6">
        <a href="{{ route('episodes.index') }}" class="flex text-white hover:underline text-base">
            <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" class="ipc-icon ipc-icon--arrow-left ipc-icon--inline ipc-link__icon" viewBox="0 0 24 24" fill="currentColor" role="presentation">
                <path d="M17.372 3.368a1.25 1.25 0 0 0-1.77 0l-8.31 8.31a.996.996 0 0 0 0 1.41l8.31 8.31c.49.49 1.28.49 1.77 0s.49-1.28 0-1.77l-7.24-7.25 7.25-7.25c.48-.48.48-1.28-.01-1.76z"></path>
            </svg> Voltar </a>

        <h1 class="text-3xl font-bold mt-4 mb-1">Ep. {{ $episode->number }} — {{ $episode->title }}</h1>

        {{-- Filtro --}}
        <div class="flex flex-wrap gap-2 my-6 text-base justify-between">
            <a href="{{ route('episodes.show', $episode) }}" class="px-3 py-1 rounded {{ !request('stars') ? 'bg-accent text-white' : 'bg-zinc-700 text-gray-300' }}">Todas</a>
            @for ($i = 10; $i >= 1; $i--)
                <a href="{{ route('episodes.show', [$episode, 'stars' => $i]) }}" class="px-3 py-1 rounded {{ request('stars') == $i ? 'bg-accent text-white' : 'bg-zinc-700 text-gray-300' }}">{{ $i }}★</a>
            @endfor
        </div>
        ...
        @auth
            <div class="bg-zinc-800 rounded-lg p-4 mb-6 border border-gray-600">
                ...
                <form action="{{ $userReview ? route('reviews.update', $userReview) : route('reviews.store', $episode) }}" method="POST" class="space-y-3">
                    ...
                    <div>
                        <label class="block text-base mb-1">Nota (1-10)</label>
                        
                        {{-- Campo de nota --}}
                        <input type="number" name="rating" min="1" max="10" value="{{ old('rating', $userReview?->rating) }}" class="bg-zinc-700 border border-gray-500 rounded w-24 p-2 text-white">
                    </div>

                    <div>
                        <label class="block text-base mb-1">Comentário</label>
                    ...
                    <button type="submit" class="bg-accent text-white px-4 py-2 rounded hover:bg-accent-hover text-base">
                ...
                @if ($userReview)
                    <form action="{{ route('reviews.destroy', $userReview) }}" method="POST" onsubmit="return confirm('Deletar sua review?')" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-400 text-base hover:underline">Deletar</button>
                    </form>
                @endif
            </div>
        @else
