...
@section('content')
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($reviews as $review)
            <div class="relative bg-zinc-800 rounded-lg shadow-md border border-gray-200 p-4">
                ...
                <h2 class="text-lg font-bold pr-16">{{ $review->title }}</h2>
                <p class="text-gray-200 mt-2 text-sm">{{ $review->content }}</p>
                @auth
                    @if (auth()->id() === $review->user_id)
                        <div class="mt-3 flex gap-2">
                            <a href="{{ route('reviews.edit', $review) }}" class="text-blue-500 text-sm hover:underline">Editar</a>
                            <form action="{{ route('reviews.destroy', $review) }}" method="POST" onsubmit="return confirm('Deletar essa review?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 text-sm hover:underline">Deletar</button>
                            </form>
                        </div>
                    @endif
                @else
                    <a href="{{ route('login') }}" class="text-gray-400 text-sm">Faça login para editar suas reviews</a>
                @endauth
            </div>
        @endforeach
    </div>
